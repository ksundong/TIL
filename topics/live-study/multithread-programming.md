...
원자적이라는 의미는 한 덩어리라는 의미입니다. 이는 그 자체로 한 동작이므로 일어나거나 일어나지 않는 둘 중의 한 경우만 해당됩니다.

이런 동작은 인터리브 될 수 없으므로 안전합니다. 하지만 메모리 일관성 오류가 발생가능해 조심해서 써야 합니다. `volatile` 키워드를 사용하면 변수가 메인 메모리에 저장되기 때문에 불일치 문제가 줄어들 수 있습니다.

간단하게 원자적인 연산을 사용하면 더 효율적일 수 있지만, 메모리 일관성 오류를 방지하기 위해 더 신경을 써야합니다.
