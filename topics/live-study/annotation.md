...
### 바이트 코드를 불러오는게 빠른가? 리플렉션을 불러오는게 빠른가?

리플렉션은 부하가 있다. 과연 그렇게 느릴까? 하지만 성능에 많은 영향을 주는건 사실이다.

메모리에 올라와 있는 정보를 읽는 것, 바이트 코드를 읽는 것...

리플렉션: 클래스 로더가 읽어오는 비용 + 메모리에 적재시키는 비용 + 내가 메모리를 읽어오는 비용

바이트 코드: 바이트 코드를 읽어오는 비용<이론상 이쪽이 빠름, 메모리도 덜 씀> 하지만 바이트 코드가 굉장히 크다면 느려질 수 있을 것 같다.

그리고 그런 것들이 문제가 되는 경우는 직접 측정을 해보는 것을 권장한다.

### Annotation Retention을 선택하는 기준
...
자바독의 극한 활용은 Mockito가 짱이다.

### Java ServiceLoader

jar파일에 있는 구현체를 가져오는 방법이 ServiceLoader

해당 코드에서 구현체를 아예 몰라도 사용할 수 있다는 점이 가장 큰 장점입니다.

이는 곧 ServiceLoader 인터페이스로 제공되고, 실제 구현체로 사용할 수 있게됩니다.

#### 어떻게 찾을 것인가?

그 jar 파일 안에다가 META-INF/services에 담아주면 된다. 인터페이스 풀패키지 경로로 파일을 만들고 안에 구현체의 풀패키지 경로를 넣어주면 된다.

사용하기 원하는 패키지는 구현체 패키지의 의존성을 등록해야 한다.

구현체 패키지는 사용하기 원하는 패키지를 의존성으로 받고, 인터페이스를 구현한다. 그 다음 resources/META-INF/services에 위와같이 작성하고, jar 패키징이나 의존성으로 만들어서 주입해준다.

#### Spring Boot에서

spring-boot-autoconfigure > META-INF/spring.factories 를 보면 비슷하게 사용하는 것을 알 수 있다.

### 개발자에게 중요한 소양

빨리 배우는 능력인데 이것을 늘리려면 기본이 되어있고, 들어본 것도 많아야 한다.

결국 짬이다.

또 하나는 탐구정신 인 것 같다. lee-maru님이 롬복을 만드신걸 보고 굉장히 칭찬하셨는데, 나도 그렇다고 생각한다. 정말 대단하다고 생각한다. 나중에 참고해봐야겠다.

### 애노테이션 프로세서를 쓴다 라는건

애노테이션을 단순하게 추가했다고 보는 것이 아니라, 애노테이션 프로세서를 직접 이용해야만 한다.

애노테이션을 리플렉션으로도 사용하는 경우가 있기 때문이다.

---

오늘은 정말 좋은 내용이 많았었고, 다시 보는 것이 좋을 것 같다.
